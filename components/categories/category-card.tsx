...
export function CategoryCard({ category }: CategoryCardProps) {
  return (
    <Card className="overflow-hidden transition-all duration-200 hover:shadow-md hover:border-slate-300 dark:hover:border-slate-600">
      <div className="h-32 flex items-center justify-center p-4" style={{ backgroundColor: category.color }}>
        <div className="relative w-full h-full flex items-center justify-center">
          {/* Category count badge */}
          <div className="absolute top-0 right-0 bg-white/20 text-white text-sm font-medium rounded-full w-8 h-8 flex items-center justify-center">
            {category.count}
          </div>
